...
const COUNTRIES = [
  ...
  { country: "Congo (Republic)", capital: "Brazzaville" },
  { country: "Congo (Democratic Republic)", capital: "Kinshasa" },
...
function initGame() {
  ...
  // Reset and shuffle countries
  remainingCountries = shuffleArray([...COUNTRIES]);
  startQuiz();
}
...
function getRandomCountry() {
  if (remainingCountries.length === 0) {
    endGame(false);
    feedbackMessageEl.textContent = "🎉 You've gone through all countries!";
    return null;
  }
  return remainingCountries.pop(); // Take one country out to prevent repeats
}


function generateOptions() {
  const correctAnswer = currentCountry.capital;
  const allCapitals = COUNTRIES.map((c) => c.capital).filter((c) => c !== correctAnswer);

  const shuffledIncorrect = shuffleArray(allCapitals);
  ...
  options.forEach((capital) => {
    ...
    button.dataset.capital = capital;
...
function nextQuestion() {
  ...
  // If no countries left, end the game
  if (!currentCountry) return;

  countryNameEl.textContent = currentCountry.country;
  generateOptions();
}
...
function handleAnswerClick(event) {
  ...
  const userAnswer = selectedButton.dataset.capital;
  const correctAnswer = currentCountry.capital;
  ...
  if (userAnswer === correctAnswer) {
    ...
    score++;
    consecutiveCorrect++;
    feedbackMessageEl.textContent = `✅ Correct! The capital of ${currentCountry.country} is ${correctAnswer}.`;
    ...
    consecutiveCorrect = 0;
    feedbackMessageEl.textContent = `❌ Incorrect. The capital of ${currentCountry.country} is ${correctAnswer}.`;

    const correctButton = Array.from(optionsContainerEl.children).find(
      (btn) => btn.dataset.capital === correctAnswer
    );
...
function endGame(didWin) {
  ...
  if (didWin) {
    countryNameEl.textContent = "🏆 WINNER! 🏆";
